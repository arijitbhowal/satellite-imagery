...
class MyDataset(Dataset):
    def __init__(self, csv_path, image_ids, patch_size, nb_dates):
        ...
        for fold in image_ids:
            rivers = []
            rivers.append(io.imread(fold + '/rivers/rivers1.tif'))
            rivers.append(io.imread(fold + '/rivers/rivers2.tif'))
            self.all_rivers.append(np.array(rivers))
    ...
    def __getitem__(self, index):
        ...
        find_rivers = self.all_rivers[image_id] [:,x:x + self.patch_size, y:y + self.patch_size]

        return np.ascontiguousarray(find_patch), np.ascontiguousarray(find_labels), np.ascontiguousarray(find_rivers)
